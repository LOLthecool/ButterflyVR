...
pub impl NetNodeServer {
    ...
    fn send_packets_server(&mut self) {
        ...
        for client in self.clients.values_mut() {
            ...
            let max_dgram_size: usize = self.networker.get_max_dgram_size(&client.conn);
            ...
            while self.message_buffer.len() > client.message_buffer_position {
                ...
                self.networker
                    .send_stream(&client.conn, *stream, message.clone());
            }
            ...
            while remaining_bandwidth > PACKET_MAX_SIZE_THRESHOLD {
                ...
                let old_map = mem::replace(&mut client.priorities, BTreeMap::new());
                ...
                if packet.len() > DGRAM_HEADER_SIZE {
                    ...
                    self.networker.send_datagram(&client.conn, packet);
                    continue;
                }
