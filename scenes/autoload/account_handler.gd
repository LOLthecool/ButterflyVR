...
func _enter_tree() -> void:
	...
	saved_token.assign(
		GlobalPersistanceHandler.register_value("user_login", "token", "token", []) as Array
	)
	var expiry: int = GlobalPersistanceHandler.register_value("user_login", "token", "expiry", -1)
	var renewable: bool = GlobalPersistanceHandler.register_value(
		"user_login",
		"token",
		"renewable",
		false,
...
func _exit_tree() -> void:
	if token_renewable:
		...
		GlobalAPIHandler \
				.make_request(HTTPClient.METHOD_GET, TOKEN_RENEW_ENDPOINT, token_header) \
				.connect(on_token_request)
