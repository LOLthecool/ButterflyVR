	...
	match values["type"]:
		...
		"AnimationNodeBlendSpace1D":
			...
			for index:int in range(0, blend_point_node_data.size()):
				...
				var sub_node:AnimationRootNode = generate_anim_node_recursive(
						data) as AnimationRootNode
		...
		"AnimationNodeStateMachine":
			...
			for transition_data:Dictionary in transitions:
				...
				@warning_ignore("unsafe_cast")
				node.add_transition(data["source"] as String, 
						data["target"] as String, transition)
			result = node
