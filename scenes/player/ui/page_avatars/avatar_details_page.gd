...
func show_details(avatar:Dictionary) -> void:
	...
	details_creation_time.text = Time.get_datetime_string_from_unix_time(avatar["created_at"] as int, true).split(" ")[0]
	@warning_ignore("unsafe_cast")
	details_update_time.text = Time.get_datetime_string_from_unix_time(avatar["updated_at"] as int, true).split(" ")[0]
	@warning_ignore("unsafe_cast")
