...
func preview_avatar(avatar: Dictionary[String, Variant]) -> void:
	...
	if !result[0]:
		...
		MiscHelpers.log_request_error(
			"error when getting avatar details",
			result[1] as int,
			result[2] as String,
			result[3] as String,
		)
	...
	@warning_ignore("unsafe_cast")
	avatar_publicity.text = "publicity: %s" % StringifyHelper.stringify_object_publicity(
		avatar["publicity"] as int
	)
	@warning_ignore("unsafe_cast")
	avatar_author.text = "created by: %s" % await APIHelper.get_username(
		UUID.from_String(avatar["creator"] as String)
	)
	@warning_ignore("unsafe_cast")
...
func on_avatar_equip() -> void:
	...
	@warning_ignore("unsafe_cast")
	avatar_handler.send_message(
		(await GlobalAccountHandler.get_uuid()).backing_storage,
		UUID.from_String(avatar["id"] as String),
	)
