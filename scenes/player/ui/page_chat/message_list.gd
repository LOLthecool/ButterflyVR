...
func _ready() -> void:
	var message_history: Array[ChatBoxManager.Message] = GlobalWorldHandler \
			.current_world \
			.chat_box_manager \
			.messages \
			.duplicate()
	...
	GlobalWorldHandler.current_world.chat_box_manager.new_message_sent.connect.call_deferred(
		on_message
	)
...
func on_message(message: ChatBoxManager.Message) -> void:
	...
	if username_cache.get(message.player) == null:
		...
		username_cache[message.player] = await APIHelper.get_username(
			UUID.from_bytes(message.player)
		)
