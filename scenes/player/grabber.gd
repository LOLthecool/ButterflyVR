...
func _physics_process(_delta: float) -> void:
	if wants_to_grab:
		...
		if is_colliding():
			grab_handler.send_message(GlobalNetworkManager.get_id(), (get_collider() as Node).get_path())
		return
	if is_grabbing:
		...
		if grabbed_node != null:
			...
			grab_handler.send_message(GlobalNetworkManager.get_id(), "")
...
func on_confirmed_grab(player:int, target:Node) -> void:
	if !(player == GlobalNetworkManager.get_id()):
		return
	if target is Node3D:
		is_grabbing = true
