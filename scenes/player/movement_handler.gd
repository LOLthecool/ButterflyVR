...
func _physics_process(delta: float) -> void:
	...
	if player_state == Player_states.CROUCHED:
		camera_rotation_origin.position.y = lerp(
			camera_rotation_origin.position.y,
			camera_height * CROUCHED_CAMERA_HEIGHT_MULTIPLIER,
			0.05,
		)
	else:
		camera_rotation_origin.position.y = lerp(
	...
	target.velocity.x = move_toward(
		target.velocity.x,
		direction.x * modified_max_speed,
		modified_speed * delta,
	)
	target.velocity.z = move_toward(
		target.velocity.z,
		direction.z * modified_max_speed,
		modified_speed * delta,
	)
